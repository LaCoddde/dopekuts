...
export function Chatbot() {
  ...
  const sendUser = (text: string) => {
    ...
    setTimeout(() => {
      setMessages((m) => [
        ...m,
        { id: crypto.randomUUID(), from: 'roy', text: "Thanks! I'm using demo data right now. When we hook the backend, I’ll take real actions for you. Anything else I can help with?" },
      ]);
    }, 600);
  };

  const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); sendUser(input); setInput(''); };

  const handleQuick = (kind: 'book' | 'services' | 'availability') => {
    let reply = '';
    if (kind === 'book') reply = "Great choice! To book an appointment, tap **Book Now** below or tell me your preferred date and time. (Demo mode)";
    else if (kind === 'services') reply = "Here are our popular services: Classic Cut, Beard Grooming, Premium Package, Express Service. What would you like to know? (Demo mode)";
    else reply = "Our typical availability is Mon–Sat, 9am–7pm. Tell me a day and I’ll check slots. (Demo mode)";
    setMessages((m) => [
      ...m,
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 z-[60] rounded-full p-4 shadow-xl transition-all
      ...
      {open && (
        ...
        >
          <div className="flex h-full flex-col">
            {/* Header (unchanged color strip) */}
            <div className="sticky top-0 z-10 flex items-center justify-between px-4 py-3 rounded-t-3xl
              ...
              <button aria-label="Close chatbot" onClick={() => setOpen(false)} className="p-2 rounded-full hover:bg-black/5 dark:hover:bg-white/10 transition">
            ...
            <div className="flex-1 overflow-y-auto px-3 pt-2 pb-4" aria-live="polite">
              {messages.map((m, i) => {
                ...
                return (
                  <div key={m.id} className={`flex mb-3 ${m.from === 'user' ? 'justify-end' : 'justify-start'} ${!isNewRoyGroup && m.from === 'roy' ? 'pl-9' : ''}`}>
